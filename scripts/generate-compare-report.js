...
function list(dir) { return fs.existsSync(dir) ? fs.readdirSync(dir) : [] }
...
function buildHtml(pairs) {
  ...
  <section class="pair">
    <h2>${p.title}</h2>
    <div class="row">
      <div class="col"><div class="label">Referência</div><img src="${p.ref}" />
</div>
...
>
    ...
    <div class="overlay" id="pair${i}">
      ...
      <div class="img-container">
        ...
        <input type="range" min="0" max="100" value="50" class="slider" oninput=
"overlay('pair${i}', this.value)" />
      </div>
    </div>
  </section>`).join('\n')
...
<head>
  ...
  <style>
    body{font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;padding:
16px;color:#111;background:#f6f7f9}
    h1{font-size:20px;margin:0 0 8px}
    .row{display:flex;gap:12px}
    .col{flex:1;border:1px solid #ddd;border-radius:8px;padding:8px;background:#
fff}
    .label{font-size:12px;color:#555;margin-bottom:4px}
    img{max-width:100%;height:auto;display:block}
    .overlay{margin-top:12px;border:1px solid #ddd;border-radius:8px;padding:8px
;background:#fff}
    .img-container{position:relative;width:100%;overflow:hidden}
    .img-container img{display:block;width:100%;height:auto}
    .img-container .top{position:absolute;top:0;left:0;width:50%;border-right:1p
x solid #f00;mix-blend-mode:difference}
    .img-container .slider{position:absolute;left:0;right:0;bottom:8px;width:40%
...
<body>
  ...
  ${sections}
  <script>function overlay(id,v){var el=document.querySelector('#'+id+' .top');
if(el) el.style.width=v+'%';}</script>
</body>
</html>`
}
...
function main() {
  ...
  if (!pages.length) {
    console.error('No pairs found. Ensure you have both ref-*-full and *-full sc
reenshots.')
    process.exit(1)
  }
  ...
  for (const p of pages) {
    ...
    if (ref && cur) {
      pairs.push({ title: p, ref: `screenshots/${ref}`.replace(/\\/g, '/'), cur:
 `screenshots/${cur}`.replace(/\\/g, '/') })
    }
  }
  if (!pairs.length) {
ed from latest files.')
    process.exit(1)
  }
